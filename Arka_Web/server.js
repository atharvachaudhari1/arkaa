...
// API Routes
app.post('/api/Arka-request', async (req, res) => {
  try {
    const {
      userName,
      userEmail,
    ...
    if (isConnected && db) {
      try {
        ...
        console.log(`✅ New Arka request saved: ${userEmail} for ${product} (${premiumType})`);
        ...
        console.log('📝 Arka Request (DB error):', JSON.stringify(requestDoc, null, 2));
      ...
      console.log('📝 Arka Request (No DB):', JSON.stringify(requestDoc, null, 2));
    ...
    console.error('Error processing Arka request:', error);
    res.status(500).json({
...
app.get('/api/Arka-requests', async (req, res) => {
  try {
    if (!isConnected || !db) {
