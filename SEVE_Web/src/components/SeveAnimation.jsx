...
import SeveLogo from './SeveLogo.jsx';
...
const SeveAnimation = ({ onAnimationComplete }) => {
  ...
  useEffect(() => {
    ...
    function startFinalSequence() {
      ...
      const seveLogo = document.getElementById('seveLogo');

      if (!mainContainer || !fadeTextContainer || !fadeText || !seveLogo) return;

      // Add final state class to container
      mainContainer.classList.add('final-state');

      // First: Show the logo with animation
      seveLogo.classList.add('visible');
    ...
    if (isMobile) {
      ...
      // Move SEVE to top after 7.5 seconds
    ...
    // Show borders after SEVE moves to top
    const mainContainer = document.getElementById('mainContainer');
    const fadeTextContainer = document.querySelector('.fade-text-container');
