...
async function ensureElevated() {
  if (process.env.Arka_SKIP_ELEVATION === '1') return true; // developer override
  if (process.env.Arka_ELEVATED === '1' || hasArg('--Arka-elevated')) return true;
  const elevated = await checkIsElevated();
  if (elevated) return true;
  try {
    const isDev = !app.isPackaged;
    const exe = process.execPath; // electron.exe (dev) or Arka Desktop.exe (prod)
    const args = [];
    if (isDev) {
    ...
    spawn('powershell', cmd, { env: { ...process.env, Arka_ELEVATED: '1' } });
    app.quit();
    return false;
  } catch (e) {
    return false;
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: 1200,
    height: 800,
    icon: path.join(__dirname, '../assets/Arka-logo.png'),
    title: 'Arka Desktop',
...
ipcMain.handle('window-close', () => {
  ...
  if (process.platform === 'win32') {
    try {
      mainWindow.setIcon(path.join(__dirname, '../assets/Arka-logo.png'));
    } catch (error) {
...
ipcMain.handle('search-files', async (_evt, { root, query }) => {
  try {
    ...
    console.log('[Arka] search-files spawn', { args, cwd: workingDir });
    const pythonExe = getPythonExecutablePath();
    const proc = spawn(pythonExe, args, {
      cwd: workingDir,
    ...
    return await new Promise((resolve) => {
      proc.on('close', code => {
        ...
        try {
          ...
          console.log('[Arka] search-files parsed', { count: results.length });
...
function getReportScriptPath() {
  const isDev = !app.isPackaged;
  if (isDev) {
    return path.join(__dirname, '..', 'report', 'generate_Arka_report.py');
  }
  return path.join(process.resourcesPath, 'report', 'generate_Arka_report.py');
}
...
ipcMain.handle('show-save-dialog-pdf', async () => {
  const result = await dialog.showSaveDialog(mainWindow, {
    defaultPath: 'Arka_Drive_Analysis_Report.pdf',
    filters: [
...
// IPC: generate Arka PDF report via Python script in /report
ipcMain.handle('generate-Arka-report', async (_evt, { filePath, data }) => {
  try {
    ...
    try {
      ...
      tempJsonPath = path.join(workingDir, 'Arka_report_input.json');
      fs.writeFileSync(tempJsonPath, JSON.stringify(enriched, null, 2), 'utf8');
      args.push('--input', tempJsonPath);
    } catch (_) { tempJsonPath = null; }

    console.log('[Arka] generate-Arka-report using pythonExe=', pythonExe, ' scriptPath=', scriptPath, ' cwd=', workingDir);
