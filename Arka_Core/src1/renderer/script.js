// Arka Desktop - Vanilla JavaScript Implementation
...
async function generateClearancePDF(report) {
    ...
    doc.text('Arka - Secure Erase & Verification Engine', 45, page.h - 40);
    const fname = `Arka_Clearance_Certificate_${new Date().toISOString().slice(0,10)}.pdf`;
    doc.save(fname);
}
...
async function generatePDFReport() {
    ...
    try {
        ...
        const res = await window.electronAPI.generateArkaReport(outPath, driveData);
        if (!res || !res.success) {
            showError(res?.error || 'Failed to generate Arka report');
            return;
        }
    } catch (e) {
        showError('Failed to generate Arka report');
    }
}
...
async function performSearch(driveId) {
    ...
    try {
        ...
        console.debug('[Arka] Searching', { driveId, normalizedRoot: root, query });
